		...
		$scope.checkout=function(){
			if($scope.total_amount>0){
				...
				var alertPopup = $ionicPopup.alert({
					title: 'لا توجد سيارات مختارة',
					template: 'من فضلك قم باختيار سيارتك أولاً'
				});
			}
		};
...
.controller('loginCtrl', function($scope,$http,$ionicPopup,$state,$ionicHistory) {
		...
		$scope.login = function() {
			str="http://192.168.43.228/carAPI/user-details.php?e="+$scope.user.email+"&p="+$scope.user.password;
			$http.get(str)
			.success(function (response){
				...
				//}
					...
					var alertPopup = $ionicPopup.alert({
						title: 'خطأ في الدخول',
						template: 'الرجاء التأكد من اسم المستخدم وكلمة المرور'
					});
			});
		};
...
.controller('signupCtrl', function($scope,$http,$ionicPopup,$state,$ionicHistory) {
	...
	$scope.signup=function(data){
			...
			var link = 'http://192.168.43.228/carAPI/signup.php';
			...
			.then(function (res){	
				...
				if($scope.response.created=="1"){
					$scope.title="تم بنجاح";
					$scope.template="تم التسجيل بنجاح... يمكنك الدخول الى حسابك الآن";
					...
					$scope.title="خطأ";
					$scope.template="البريد المدخل موجود مسبقا اذا فقدت كلمة المرور اتصل على 888888";
				
				}else{
					$scope.title="خطا";
					$scope.template="خطأغير مقصود اتصل بالدعم على 88888";
				}
   ...
   .controller('bayCtrl', function($scope,$http,$ionicPopup,$state,$ionicHistory) {

	$scope.signup=function(data){
			
			var link = 'http://192.168.43.228/carAPI/bay.php';
			$http.post(link, {n : data.name, un : data.username, ps : data.password , ph: data.phone , add : data.address , pin : data.pincode })
			.then(function (res){	
				$scope.response = res.data.result; 
				...
				if($scope.response.created=="1"){
					...
					$scope.title="خطأ";
					$scope.template="خطأ يمكنك المحاولة مرة اخرى";
...
.controller('productPageCtrl', function($scope) {
	...
	angular.element(document).ready(function () {
		...
		$scope.img= "http://192.168.43.228/img/"+ sessionStorage.getItem('product_info_img')+"";
